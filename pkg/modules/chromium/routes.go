...
// PDF. If a data.json file is provided, the HTML template will be rendered
// with the JSON data before conversion.
func convertHtmlRoute(chromium Api, engine gotenberg.PdfEngine) api.Route {
	return api.Route{
		...
		Handler: func(c echo.Context) error {
			...
			var url string
			if dataPath != "" {
				// Apply JSON data to HTML template.
				url, err = jsonToHtml(ctx, inputPath, dataPath)
				if err != nil {
					return fmt.Errorf("apply JSON data to template: %w", err)
				}
			} else {
...
// TemplatesDir is the directory where internal templates are stored.
const TemplatesDir = "/opt/gotenberg/templates"

// convertTemplateRoute returns an [api.Route] which can convert a stored template
// with JSON data to PDF. The template name and JSON data are provided as form fields.
func convertTemplateRoute(chromium Api, engine gotenberg.PdfEngine) api.Route {
	return api.Route{
		Method:      http.MethodPost,
		Path:        "/forms/chromium/convert/template",
		IsMultipart: true,
		Handler: func(c echo.Context) error {
			ctx := c.Get("context").(*api.Context)
			form, options := FormDataChromiumPdfOptions(ctx)
			...
			var jsonData string
			err := form.
				MandatoryString("template", &templateName).
				MandatoryString("json", &jsonData).
				Validate()
			if err != nil {
				return fmt.Errorf("validate form data: %w", err)
			}
			...
			if _, err := os.Stat(templatePath); os.IsNotExist(err) {
				return api.WrapError(
					...
					api.NewSentinelHttpError(
						...
						fmt.Sprintf("Template '%s' not found in %s", templateName, TemplatesDir),
					),
				)
			}

			// Apply JSON data to template.
			url, err := templateToHtmlFromString(ctx, templatePath, jsonData)
			if err != nil {
				return fmt.Errorf("apply JSON data to template: %w", err)
			}
			...
			err = convertUrl(ctx, chromium, engine, url, options, mode, pdfFormats, metadata, userPassword, ownerPassword, embedPaths)
			if err != nil {
				return fmt.Errorf("convert template to PDF: %w", err)
			}
...
// templateToHtml applies JSON data to a stored HTML template.
func templateToHtml(ctx *api.Context, templatePath string, dataPath string) (string, error) {
	// Read the JSON data file.
	jsonBytes, err := os.ReadFile(dataPath)
	if err != nil {
		return "", fmt.Errorf("read JSON data file: %w", err)
	}
	...
	if err != nil {
		return "", api.WrapError(
			fmt.Errorf("parse JSON data: %w", err),
			api.NewSentinelHttpError(
				http.StatusBadRequest,
				fmt.Sprintf("Invalid JSON in data.json: %s", err),
			),
		)
	}
	...
	if err != nil {
		return "", api.WrapError(
			fmt.Errorf("parse HTML template: %w", err),
			api.NewSentinelHttpError(
				http.StatusBadRequest,
				fmt.Sprintf("Invalid HTML template syntax: %s", err),
			),
		)
	}
	...
	if err != nil {
		return "", api.WrapError(
			fmt.Errorf("execute template: %w", err),
			api.NewSentinelHttpError(
				http.StatusBadRequest,
				fmt.Sprintf("Template execution error: %s", err),
			),
		)
	}
	...
	err = os.WriteFile(outputPath, buffer.Bytes(), 0o600)
	if err != nil {
		return "", fmt.Errorf("write template result: %w", err)
	}

	return fmt.Sprintf("file://%s", outputPath), nil
}

// templateToHtmlFromString applies JSON data (as a string) to a stored HTML template.
func templateToHtmlFromString(ctx *api.Context, templatePath string, jsonData string) (string, error) {
	// Parse JSON into a map.
	var data map[string]interface{}
...
// jsonToHtml applies JSON data to an HTML template using Go's html/template.
// The template can use Go template syntax like {{ .fieldName }}.
func jsonToHtml(ctx *api.Context, inputPath string, dataPath string) (string, error) {
	// Read the JSON data file.
	jsonBytes, err := os.ReadFile(dataPath)
	if err != nil {
		return "", fmt.Errorf("read JSON data file: %w", err)
